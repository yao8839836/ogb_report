...
class KGEModel(nn.Module):
    def __init__(self, model_name, nentity, nrelation, hidden_dim, gamma, evaluator,
        ...
        anchor_file = 'dataset/ogbl_wikikg2/ogbl-wikikg2_20000_anchors_20000_paths_d0.4_b0.0_p0.4_r0.2_pykeen_50sp_bfs.pkl'
        neighbor_file = 'dataset/ogbl_wikikg2/neighbors+relations_with_max_degrees.pkl'
        ...
        if self.node_itself or self.sample_neighbors > 0:
            ...
            ent_feat = np.loadtxt("dataset/ogbl_wikikg2/mapping/nodeidx2entityid_sent_emb.txt")
            print(ent_feat)
            print(ent_feat.shape)
            
            padding_emb = np.array([[0.0 for x in  range(768)]])
            c = np.r_[ent_feat, padding_emb]

            weight = torch.from_numpy(c).float()

            #weight = torch.FloatTensor([[1, 2.3, 3], [4, 5.1, 6.3]])
            #embedding = nn.Embedding.from_pretrained(weight)

            self.node_embeddings = nn.Embedding.from_pretrained(weight)
